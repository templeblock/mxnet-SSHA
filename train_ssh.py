...
from rcnn.symbol import symbol_ssh_ssd
from rcnn.symbol import symbol_ssh
from rcnn.symbol import symbol_ssh_pvanet_like
from rcnn.symbol import symbol_ssh_pvanet_like_ssd
...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,
    ...
    if args.symbol == "symbol_ssh":
        sym = symbol_ssh.get_ssh_train()
    elif args.symbol == "symbol_ssh_ssd":
        sym = symbol_ssh_ssd.get_ssh_train()
    elif args.symbol == "symbol_ssh_pvanet_like_ssd":
        sym = symbol_ssh_pvanet_like_ssd.get_ssh_train()
    else:
        print("Not supported symbol", args.symbol)
    ...
    if args.resume:
        ...
        print("arg_params = 0; aux_params = 0")
        arg_params = dict()
        aux_params = dict()
        for k,v in arg_shape_dict.iteritems():
          if k.find('upsampling')>=0:
    ...
    def _batch_callback(param):
      ...
      save_step[0] +=1
      if save_step[0]%2000 == 0:
          save_model(save_step[0]/2000)
