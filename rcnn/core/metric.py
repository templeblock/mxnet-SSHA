...
class RPNAccMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        # print("RPNACC pred: ", pred.shape, self.pred_idx)
        # print("RPNACC label: ", label.shape, self.label_idx)
...
class RPNL1LossMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        # print("RPNL1 bbox_loss: ", bbox_loss.shape)
        # print("RPNL1 bbox_weight: ", bbox_weight.shape)
...
class RPNL1LossMetricKpoint(mx.metric.EvalMetric):
    def __init__(self, loss_idx=-1, weight_idx=-1):
        super(RPNL1LossMetricKpoint, self).__init__('RPNL1Loss')
        self.pred, self.label = get_rpn_names()
        self.loss_idx = loss_idx
        self.weight_idx = weight_idx
    ...
    def update(self, labels, preds):
        if self.loss_idx>=0 and self.weight_idx>=0:
          ...
          bbox_loss = preds[self.pred.index('rpn_kpoint_loss')].asnumpy()
          bbox_weight = labels[self.label.index('rpn_kpoint_weight')].asnumpy()
          #bbox_weight = preds[self.pred.index('rpn_bbox_weight')].asnumpy()

        # print("RPNL1 bbox_loss: ", bbox_loss.shape)
        # print("RPNL1 bbox_weight: ", bbox_weight.shape)
        # calculate num_inst (average on those fg anchors)
        num_inst = np.sum(bbox_weight > 0) / 10
        #print('in_metric log', bbox_loss.shape, num_inst, file=sys.stderr)
