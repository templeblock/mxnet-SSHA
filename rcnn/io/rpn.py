...
def assign_anchor_fpn(feat_shape, gt_boxes, gt_points, im_info):
    ...
    bbox_targets = np.zeros((len(inds_inside), 4), dtype=np.float32)
    kpoint_targets = np.zeros((len(inds_inside), 10), dtype=np.float32)
    if gt_boxes.size > 0:
        bbox_targets[:] = nonlinear_transform(anchors, gt_boxes[argmax_overlaps, :4])
        kpoint_targets[:] = kpoint_transform(anchors, gt_points[argmax_overlaps, :10])

    bbox_weights = np.zeros((len(inds_inside), 4), dtype=np.float32)
    bbox_weights[labels == 1, :] = np.array(config.TRAIN.RPN_BBOX_WEIGHTS)

    kpoint_weights = np.zeros((len(inds_inside), 10), dtype=np.float32)
    kpoint_weights[labels ==1, :] = np.array(config.TRAIN.RPN_KPOINT_WEIGHTS)
    if gt_points.size > 0:
        gt_points_flag = gt_points[argmax_overlaps, 10]
        gt_points_flag = np.array(gt_points_flag, np.int)
        kpoint_weights[gt_points_flag == 0, :] = np.array(config.TRAIN.RPN_KPOINT_WEIGHTS_NON)
