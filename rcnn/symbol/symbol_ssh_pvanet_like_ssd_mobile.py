...
def get_ssh_conv(data):
    ...
    conv1 = mx.symbol.Convolution(
        data=data, kernel=(3,3), pad=(1,1), stride=(2,2), num_filter=24, workspace=2048, name="conv1")
    ...
    m1 = ssh_detection_module(inception1, 256, 'ssh_m1_det')

    conv3_1 = mx.symbol.Convolution(data=m1, kernel=(1,1), num_filter=128, name="conv3_1")
    ...
    m2 = ssh_detection_module(conv3_2_relu, 256, 'ssh_m2_det')

    conv4_1 = mx.symbol.Convolution(data=m2, kernel=(1, 1), num_filter=128, name="conv4_1")
    ...
    m3 = ssh_detection_module(conv4_2_relu, 256, 'ssh_m3_det')

    if FPN:
        m1, m2, m3 = get_feat_down([m3, m2, m1])

    F1 = 256
    F2 = 128
    _bwm = 1.0
    conv4_128 = conv_act_layer(m1, 'ssh_m1_red_conv',
        F2, kernel=(1, 1), pad=(0, 0), stride=(1, 1), act_type='relu', bias_wd_mult=_bwm)
    conv5_128 = conv_act_layer(m2, 'ssh_m2_red_conv',
        F2, kernel=(1, 1), pad=(0, 0), stride=(1, 1), act_type='relu', bias_wd_mult=_bwm)
