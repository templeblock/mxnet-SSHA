...
def kpoint_transform(ex_rois, gt_rois):
    """
    compute bounding box regression targets from ex_rois to gt_rois
    :param ex_rois: [N, 4]
    :param gt_rois: [N, 4]
    ...
    targets_p1_dx = (gt_rois[:, 0] - ex_ctr_x) / (ex_widths + 1e-14)
    targets_p2_dx = (gt_rois[:, 2] - ex_ctr_x) / (ex_widths + 1e-14)
    targets_p3_dx = (gt_rois[:, 4] - ex_ctr_x) / (ex_widths + 1e-14)
    targets_p4_dx = (gt_rois[:, 6] - ex_ctr_x) / (ex_widths + 1e-14)
    targets_p5_dx = (gt_rois[:, 8] - ex_ctr_x) / (ex_widths + 1e-14)
    targets_p1_dy = (gt_rois[:, 1] - ex_ctr_y) / (ex_heights + 1e-14)
    targets_p2_dy = (gt_rois[:, 3] - ex_ctr_y) / (ex_heights + 1e-14)
    targets_p3_dy = (gt_rois[:, 5] - ex_ctr_y) / (ex_heights + 1e-14)
    targets_p4_dy = (gt_rois[:, 7] - ex_ctr_y) / (ex_heights + 1e-14)
    targets_p5_dy = (gt_rois[:, 9] - ex_ctr_y) / (ex_heights + 1e-14)
